...
class CommentServices():
    ...
    def update_comment(comment_id, user_id, validated_data, user=None):
        ...
        with transaction.atomic():
            ...
            return CommentServices.get_comment_by_id(comment_id, user=user)
    ...
    def get_comment_by_id(comment_id, user=None):
        ...
        try:
            r = get_redis_client()
            members = r.smembers("global_review_comment_likes_buffer")
            if members:
                for m in members:
                    try:
        ...
        if user and user.is_authenticated:
            ...
            liked_in_db = ReviewCommentLikeRepo.get_liked_comment_ids_by_user(user.id, [comment.id])
            liked_in_redis = False
            try:
                r = get_redis_client()
                liked_in_redis = bool(r.sismember("global_review_comment_likes_buffer", f"{user.id}:{comment.id}"))
            except Exception:
                pass
    ...
    @staticmethod
    def get_comments_by_review(review_id, page, page_size, user=None):
        if not ReviewRepo.get_review_by_id(review_id):
            raise NotFoundError(f"Review with id {review_id} not found.")

        result = ReviewCommentRepo.get_comments_by_review(review_id, page, page_size)
        ...
        if comments:
            ...
            if user and user.is_authenticated:
                from review.db.repositories.review_comment_like_repo import ReviewCommentLikeRepo
                liked_in_db = ReviewCommentLikeRepo.get_liked_comment_ids_by_user(user.id, comment_ids)
                ...
                try:
                    ...
                    for c_id in comment_ids:
                        pipe.sismember("global_review_comment_likes_buffer", f"{user.id}:{c_id}")
