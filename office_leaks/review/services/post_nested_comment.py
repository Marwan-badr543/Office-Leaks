...
class PostNestedCommentServices():

    @staticmethod
    def validate_user_and_parent_comment_exist(user_id, parent_comment_id):
        if not PostCommentRepo.get_comment_by_id(parent_comment_id):
            raise NotFoundError(f"Post comment with id {parent_comment_id} not found.")
    ...
    def create_comment(validated_data):
        ...
        PostNestedCommentServices.validate_user_and_parent_comment_exist(user_id, parent_comment_id)

        parent_comment = PostCommentRepo.get_comment_by_id(parent_comment_id)
        validated_data['parent_comment_author_id'] = parent_comment.user_id

        post_id = parent_comment.post_id

        with transaction.atomic():
            comment = PostNestedCommentRepo.create_comment(validated_data)
    ...
    def update_comment(comment_id, user_id, validated_data, user=None):
        comment = PostNestedCommentRepo.get_comment_by_id(comment_id)
        if not comment:
        ...
        with transaction.atomic():
            PostNestedCommentRepo.update_comment(comment_id, validated_data)
            return PostNestedCommentServices.get_comment_by_id(comment_id, user=user)
    ...
    def get_comment_by_id(comment_id, user=None):
        ...
        try:
            r = get_redis_client()
            members = r.smembers("global_post_nested_comment_likes_buffer")
            if members:
                for m in members:
                    try:
        ...
        if user and user.is_authenticated:
            ...
            liked_in_db = PostNestedCommentLikeRepo.get_liked_nested_comment_ids_by_user(user.id, [comment.id])
            liked_in_redis = False
            try:
                r = get_redis_client()
                liked_in_redis = bool(r.sismember("global_post_nested_comment_likes_buffer", f"{user.id}:{comment.id}"))
            except Exception:
                pass
    ...
    def get_comments_by_parent(parent_comment_id, page, page_size, user=None):
        ...
        result = PostNestedCommentRepo.get_comments_by_parent(parent_comment_id, page, page_size)
        ...
        if comments:
            ...
            if user and user.is_authenticated:
                from review.db.repositories.post_nested_comment_like_repo import PostNestedCommentLikeRepo
                liked_in_db = PostNestedCommentLikeRepo.get_liked_nested_comment_ids_by_user(user.id, comment_ids)
                ...
                try:
                    ...
                    for c_id in comment_ids:
                        pipe.sismember("global_post_nested_comment_likes_buffer", f"{user.id}:{c_id}")
    ...
    def delete_comment(comment_id, user_id):
        ...
        post_id = comment.parent_comment.post_id

        with transaction.atomic():
            PostNestedCommentRepo.delete_comment(comment)
