...
from ..db.models import ReviewNestedComment


class ReviewNestedCommentCreateSerializer(serializers.ModelSerializer):
    user_id = serializers.IntegerField(write_only=True)
    parent_comment_id = serializers.IntegerField(write_only=True)

    class Meta:
        model = ReviewNestedComment
        fields = ['user_id', 'parent_comment_id', 'comment_title']


class ReviewNestedCommentUpdateSerializer(serializers.Serializer):
    comment_title = serializers.CharField()


class ReviewNestedCommentReadSerializer(serializers.ModelSerializer):
...
class GetReviewNestedCommentsSerializer(serializers.Serializer):
    parent_comment_id = serializers.IntegerField()
    page_size = serializers.IntegerField(min_value=1, max_value=100)
    page = serializers.IntegerField(min_value=1)


class DeleteReviewNestedCommentSerializer(serializers.Serializer):
